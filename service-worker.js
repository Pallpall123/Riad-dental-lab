const CACHE_NAME = "riad-dental-lab-cache-v1";
...
self.addEventListener("fetch", (event) => {
  ...
  const isPage = event.request.mode === "navigate" || event.request.url.endsWith("index.html") || event.request.url.endsWith("/Riad-dental-lab/");
  if (isPage) {
    event.respondWith(
      fetch(event.request)
        .then((response) => {
